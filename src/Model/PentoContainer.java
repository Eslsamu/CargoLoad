...
//		if(containedParcels.size() >= (containerX*containerY*containerZ/5) -1|| iteration <= 0) {
//			printContainer();
//			System.out.println(containedParcels.size());
//			return true;
//		}
		printContainer();
		if (startTimer) {
...
//            printContainedShapes();
	...
	public boolean doesFit(PentominoShape p, Coordinates c) {
		for(Monimo m : p.getChildren()) {
			...
			if(xfit > containerX - 1 || yfit > containerY- 1|| zfit > containerZ - 1
